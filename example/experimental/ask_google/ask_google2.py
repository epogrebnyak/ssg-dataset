...
def find_div(text):
    ...
    return (
        ...
        .replace("\xa0", " ")
    )  # this will give you the outer text which is like 'About 1,410,000,000 results'
...
def parse(text):
    ...
    results_num = "".join(
        [s for s in total_results_text if s.isdigit()]
    )  # now will clean it up and remove all the characters that are not a number .
